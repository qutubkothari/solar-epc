...
export async function POST(request: Request) {
  try {
    ...
    const marginTotal = lineItems.reduce(
      (sum: number, line) => sum + Number(line.rate) * percentToDecimal(Number(line.marginPercent)) * Number(line.quantity),
      0
    );
    const taxTotal = lineItems.reduce(
      (sum: number, line) => sum + Number(line.rate) * percentToDecimal(Number(line.taxPercent)) * Number(line.quantity),
      0
    );
