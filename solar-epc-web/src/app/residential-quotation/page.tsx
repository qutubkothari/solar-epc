...
import { ResidentialQuotationForm } from "@/components/residential-quotation-form";
...
import { resolveResidentialBoqItemHead, RESIDENTIAL_BOQ_SEQUENCE } from "@/lib/residential-boq";
...
export default function ResidentialQuotationsPage() {
  ...
  const editingBoqRows = useMemo(() => {
    ...
    return editingVersion.items
      .map((entry) => {
        ...
        const itemHead = resolveResidentialBoqItemHead(item);
        const seqConfig = RESIDENTIAL_BOQ_SEQUENCE.find(cfg => cfg.itemHead === itemHead);
        const sequence = seqConfig ? seqConfig.sequence : 0;
  ...
  return (
    <div className="space-y-6">
      <SectionHeader
        title="Residential Quotation Engine"
